$(document).ready(function(e) {
			...
			/*	$(".btwen_text").val("题目");
				...
				$(".swcbj_but").live("click", function() {
					var list = document.getElementsByClassName("nmb");
					var Grade = $(".yd_box").find(".movie_box").length;
					console.log(Grade);
					// questionID = list[list.length-1].innerHTML;
					...
					var domvalueA = [];
					var wenbkDoms = document.querySelectorAll('.input_wenbk');
					for(var i=0; i<wenbkDoms.length; i++){
						domvalueA.push(wenbkDoms[i].value);
					}
					console.log(domvalueA);
					$.ajax({
							'type':'get',
							'url':'ques_eachtable.php',
							'dataType':'json',
							'data':{'Grade':Grade,'surveyID':surveyID,'data':domvalueA,'questionType':questionType},
							'succes':function(res){
   											alert(res.s);
											},
